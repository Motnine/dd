...
pub async fn apply(
    ...
    // Обновляем денормализованные счётчики в таблице artists
    let all_ids: Vec<Uuid> = primary_ids
        .iter()
        .chain(featured_ids.iter())
        .chain(producer_ids.iter())
        .chain(remixer_ids.iter())
        .copied()
        .collect();
    ...
    for artist_id in &all_ids {
        sqlx::query(
            "UPDATE artists SET
                track_count_primary = (
                    SELECT COUNT(*)::int4 FROM track_artists
                    WHERE artist_id = $1 AND role = 'primary'
                ),
                track_count_featured = (
                    SELECT COUNT(DISTINCT track_id)::int4 FROM track_artists
